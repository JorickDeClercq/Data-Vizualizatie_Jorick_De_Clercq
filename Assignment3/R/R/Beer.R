...
parentKey=2433406 ## Ursus
## although we are specifically interested in Ursus Americanus and Ursus Arctos, we download all Ursus taxa so that we can use the locations of other species as absences in our species distribution modelling

this=cached_occ_search(taxonKey=parentKey,continent="north_america",limit=10000)$data

## quick check
md=map_data("world")
p=ggplot(data=subset(this,grepl("Ursus (am|ar)",name)),aes(x=decimalLongitude,y=decimalLatitude))+geom_point(aes(color=name))
...
trainenv$ov=as.factor(grepl("Ursus americanus",this$name))
trainenv$oh=as.factor(grepl("Ursus arctos",this$name))
...
r2stl(gridlon,gridlat,temp,filename="Ursus_americanus.stl")

temp=matrix(grid_data$oh,nrow=length(gridlon))+offset_height
temp[is.na(temp)]=0
temp=temp+offset_height
r2stl(gridlon,gridlat,temp,filename="Ursus_arctos.stl")
